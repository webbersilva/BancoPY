...
def criar_conta() -> None:
    ...
    cpf: str = input('CPF fo cliente')
    data_nascimento: str = input('Data de nascimento do cliente: ')

    cliente: Cliente = Cliente(nome, email, cpf, data_nascimento)

    conta: Cliente = Cliente(nome, email, cpf, data_nascimento)
...
def efetuar_deposito() -> None:
    if len(contas) > 0:
        ...
        if conta:
            valor: float = float(input('Informe o valor do déposito: '))
    ...
    if len(contas) > 0:
        ...
        if conta_o:
            ...
            if conta_d:
                ...
                print(f'A conta destino com número {numero_d} não foi encontrado.')
        else:
            print(print(f'A sua conta com número {numero_o} não foi encontrada.'))
    else:
        print(f'Ainda não existem contas cadastradas.')
    sleep(2)
    menu()
